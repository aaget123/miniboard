...
function drawFlowchart(board: Board, args: Record<string, unknown>): string {
  ...
  // 布局方向与间距参数（LR = 自左向右；间距 clamp 到合理区间防极端值）
  const isLR = args.direction === "LR";
  const hGap = Math.min(200, Math.max(20, numOf(args.hGap) ?? H_GAP));
  const vGap = Math.min(300, Math.max(20, numOf(args.vGap) ?? V_GAP));
  ...
  // 按层分组：层内沿排列轴（TB 横排 / LR 纵排）居中，层间沿流向轴堆叠
  const layers = new Map<number, { id: string; label: string }[]>();
  const layerSizes = new Map<number, number>();
  // 层内排列方向的节点尺寸与间距 / 流向轴的节点尺寸与层间距
  const alongNode = isLR ? NODE_H : NODE_W;
  const alongGap = isLR ? vGap : hGap;
  const crossNode = isLR ? NODE_W : NODE_H;
  const crossGap = isLR ? hGap : vGap;
  for (const n of validNodes) {
    ...
    layers.set(l, list);
    layerSizes.set(l, list.length * alongNode + (list.length - 1) * alongGap);
  }
  const maxLayerSize = Math.max(...[...layerSizes.values()]);
  ...
  if (els.length) {
    ...
    originX = inner.x - maxLayerSize / 2;
    originY = inner.y - 120;
  }
  ...
  for (const [l, list] of [...layers.entries()].sort((a, b) => a[0] - b[0])) {
    const layerSize = layerSizes.get(l) ?? 0;
    // 流向轴坐标：层号 ×（节点流向尺寸 + 层间距）；排列轴：居中对齐
    const flowPos = (isLR ? originX : originY) + l * (crossNode + crossGap);
    const startAlong = (isLR ? originY : originX) + (maxLayerSize - layerSize) / 2;
    list.forEach((n, i) => {
      const alongPos = startAlong + i * (alongNode + alongGap);
      const x = isLR ? flowPos : alongPos;
      const y = isLR ? alongPos : flowPos;
  ...
  for (const e of edges) {
    ...
    // 连线锚点：跨层沿流向从出口边指向入口边；同层从侧面错位相连
    const sameLayer = level.get(e.to) === level.get(e.from);
    let sx: number;
    let sy: number;
    let tx: number;
    let ty: number;
    if (!isLR) {
      sx = from.x + NODE_W / 2;
      sy = from.y + (sameLayer ? NODE_H / 2 : NODE_H);
      tx = to.x + NODE_W / 2 + (sameLayer ? (to.x > from.x ? -NODE_W / 2 : NODE_W / 2) : 0);
      ty = to.y + (sameLayer ? NODE_H / 2 : 0);
    } else {
      sx = from.x + (sameLayer ? NODE_W / 2 : NODE_W);
      sy = from.y + NODE_H / 2;
      tx = to.x + (sameLayer ? NODE_W / 2 : 0);
      ty = to.y + NODE_H / 2 + (sameLayer ? (to.y > from.y ? -NODE_H / 2 : NODE_H / 2) : 0);
    }
  ...
  return `已生成流程图（${isLR ? "从左到右" : "从上到下"}）：${validNodes.length} 个节点、${edges.length} 条连线，起点位于画布 (${Math.round(originX)}, ${Math.round(originY)})。连线为正交折线并已绑定节点。节点 id 映射：${mapping}（后续可用这些 id 引用）`;
}
...
export async function executeTool(
  ...
  switch (tool.name) {
    case "ask_user": {
      // 澄清提问正常由面板在工具循环层拦截等待用户回答；
      // 走到这里说明调用方未接管（如测试环境）
      return {
        name: tool.name,
        args,
        result: "错误：当前界面无法向用户提问",
        changed: false,
      };
    }
