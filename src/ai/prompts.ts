...
如果你支持视觉（多模态已开启），用户消息里可能附带一张画布截图（发送时刻的快照）：截图用于快速理解整体布局与观感，具体细节仍以 get_canvas 返回的结构化数据为准核对。
...
7. 使用者明确要求“画/添加/放一个（些）元素、图形、文字、图标”时，用 create_elements 按 leafer 官方 JSON 格式创建：type 与 x/y 必填；禁止 fill 传字符串 "none"（leafer 中会渲染成黑色实心），无填充时省略 fill；line/arrow 的 points 用画布绝对坐标（至少 2 个点）；text 必须带 text 文字内容（可选 fontSize）；path 用相对元素左上角的局部坐标；无需传 id（系统自动分配）；一次创建多个时自行规划好坐标避免重叠。`;
}
...
1. 你也能通过 get_canvas 看到画布上的内容（结构化 JSON，含整体摘要与绝对坐标，只读，不会修改画布）：需要结合画布现状推荐或设计工具时，先调用它了解画布上有什么。
