...
export type ToolbarPaneDeps = {
  /** 工具注册表：行渲染/布局计算的数据源 */
  registry: ToolRegistry;
  /** 工具栏布局变更回调（main.ts 转 toolbar.setVisible；自定义组定义变化时一并同步顶栏） */
  onToolbarChange: (visible: string[] | null, customGroups?: CustomGroupDef[]) => void;
};

/**
 * 「工具栏」页签控制器：自定义顶栏布局（勾选平铺 + 指针拖拽排序 + 自定义分组管理）。
 * 与顶栏同构分区展示——平铺区（拖拽排序）+ 分组收纳区（与顶栏下拉一一对应）+ 隐藏区，
 * 预览条与真实顶栏共用同一布局计算（computeToolbarNodes + foldIntoMore），所见即所得。
 * 指针拖拽自行实现（WebView2 中原生 DnD 不可靠）；持久化经 ./toolbar 的偏好存取。
 */
export class ToolbarPaneController {
  ...
  /** 指针拖拽状态：源 + 起点（超过阈值后激活，避免与点击/勾选冲突） */
  private dragState: {
    src: string;
  ...
  constructor(
    ...
    private readonly deps: ToolbarPaneDeps,
  ) {
    // 实时预览：当前平铺顺序所见即所得（勾选/排序即时刷新）
    this.previewEl = document.createElement("div");
    this.previewEl.className = "tb-preview";
    container.appendChild(this.previewEl);

    this.listEl = document.createElement("div");
    this.listEl.className = "tb-list";
    ...
    resetBtn.textContent = "恢复默认布局";
    resetBtn.addEventListener("click", () => this.resetToolbarLayout());
    actions.appendChild(resetBtn);
    container.appendChild(actions);
  }

  /** 打开弹窗时同步：关闭残留的“+”添加面板并整页重建 */
  refresh() {
    this.groupAddOpen = null;
    this.renderToolbarPane();
  }

  // ---------- 拖拽引擎（指针事件自行实现，替代原生 DnD） ----------
  ...
  private onDragMove = (e: PointerEvent) => {
    ...
    const r = this.listEl.getBoundingClientRect();
    if (e.clientY < r.top + 28) {
      this.listEl.scrollTop -= 14;
  ...
  private clearDragOver() {
    for (const el of this.listEl.querySelectorAll(".drag-over")) {
  ...
  private moveGroupBefore(src: string, targetG: string, after = false) {
    ...
    this.renderToolbarPane();
    // 通知顶栏重渲染（分组按钮顺序变化）
    this.deps.onToolbarChange(this.toolbarPref);
  }
  ...
  private renderToolbarPane() {
    ...
    for (const t of this.deps.registry.list()) {
    ...
    const rest = this.deps.registry.list().filter((t) => !pinnedSet.has(t.id));
    ...
    for (const item of pinnedItems) {
      if (isGroupMarker(item)) {
        ...
        const gTools = this.deps.registry
          .list()
        ...
        const t = this.deps.registry.getTool(item);
        if (t) {
          pinnedSection.appendChild(this.makeToolbarRow(t, true));
        }
      }
    }
    this.listEl.appendChild(pinnedSection);
    ...
    this.listEl.appendChild(groupsTitle);
    // 新建/重命名内联输入行（不依赖 window.prompt，Tauri WebView 不支持原生 prompt）
    if (this.groupEdit) {
      this.listEl.appendChild(this.makeGroupEditRow());
    }
  ...
  private makeGroupEditRow(): HTMLElement {
    ...
    const commit = () => {
      ...
      this.renderToolbarPane();
      this.deps.onToolbarChange(this.toolbarPref, this.customGroups);
    };
  ...
  private makeGroupAddPanel(g: ToolGroup): HTMLElement {
    ...
    const candidates = this.toolbarVisible
      ...
      .map((id) => this.deps.registry.getTool(id))
  ...
  private renderToolbarPreview() {
    ...
    for (const node of nodes) {
      if (node.kind === "tool") {
        const t = this.deps.registry.getTool(node.id);
        if (!t) {
          continue;
        }
    ...
    this.previewEl.appendChild(moreMenu);
    ...
    if (moreItems.size) {
      moreBtn.title = `更多▾：顶栏放不下时自动收纳（${[...moreItems.keys()]
        .map((id) => this.deps.registry.getTool(id)?.name ?? id)
        .join(" / ")}）`;
    }
    ...
    this.previewEl.appendChild(styleItem);
    ...
    this.previewEl.appendChild(fillItem);
  }
  ...
  private applyToolbarLayout() {
    ...
    this.deps.onToolbarChange(visible);
    this.renderToolbarPane();
  }
  ...
  private resetToolbarLayout() {
    ...
    void showConfirm({
      ...
      this.deps.onToolbarChange(null);
      this.renderToolbarPane();
    });
  }
}
