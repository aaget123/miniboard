...
- **整理**：智能识别手绘形状并完善为标准图形——闭合路径按形状识别为圆 / 椭圆 / 矩形（支持旋转）/ 三角形 / 四边形 / 五边形等标准元素，近似直线转为标准线段，其余弯曲线条做拉直简化（Douglas-Peucker，容差 5px）；保留颜色与粗细，可继续拖动调整（画笔笔迹以原始采样点为识别源，识别为椭圆/直线后可直接编辑）
- **手绘风格**（✎ 按钮）：选中标准图形（矩形 / 椭圆 / 直线 / 箭头 / 多边形）一键转为 rough.js 手绘外观（双线 + 抖动描边），抖动 seed 随元素保存、撤销重载后完全可复现；AI 感知时能识别出原始几何形状；对已手绘元素重复点击自动跳过
- **画布缩放**：自由缩放 10% ~ 800%
