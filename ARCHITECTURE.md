...
- ✅ settings 页签模块化 ✓ 收官：shortcuts-pane / prompt-pane / data-pane /
  toolbar-pane / model-pane 五个页签控制器（DOM 自建 + 刷新收口，设置弹窗
  只留门面），settings.ts 2153→435 行；settings 冒烟 E2E 守护
  （tests/e2e/settings-smoke.cjs，14 项，含配置增删/取消平铺联动/恢复默认）
